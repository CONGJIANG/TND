...
modEIF1<-function(dat){
  TNDmod_g<-glm(V~C + sin(C),family=binomial(),data=dat)
  g1_all=predict(TNDmod_g,type="response")
  g0_all= 1 - g1_all
  ...
  Q1=predict(TNDmod,newdata=as.data.frame(cbind(V=1,C=dat$C)),type="response")*w1
  Q0=predict(TNDmod,newdata=as.data.frame(cbind(V=0,C=dat$C)),type="response")*w0
...
for (i in 1:1000){
  ...
  est4<-modEIF4(dat)
  write(c(i,est1,est2,est3,est4),file="/Users/congjiang/Downloads/Study2results4v2.txt",ncolumns=40,append=T)
}


res1<-read.table("/Users/congjiang/Downloads/Study2results4v2.txt",header=F)
head(res1)
dim(res1)
#em=0

...
mod_mu<-function(TNDdf_train){
  ...
  Out_mu <- earth(
    ...
    nprune = as.numeric(tuned_mars$bestTune)[1],
    degree = as.numeric(tuned_mars$bestTune)[2]
  )
...
mod_g<-function(TNDdf_train){
    # tuning
    tuned_mars <- train(
      x = subset(TNDdf_train, select = c(-Y,-V)),
      y = TNDdf_train$V,
      method = "earth",
      trControl = trainControl(method = "cv", number = 10),
      tuneGrid = hyper_grid
    ...
    TNDmod_g <- earth(
      V ~ .,  
      data = subset(TNDdf_train, select = -Y),
      glm=list(family=binomial),
      nprune = as.numeric(tuned_mars$bestTune)[1],
      degree = as.numeric(tuned_mars$bestTune)[2]
    )
...
mod_g_control<-function(TNDdf_train){
  ...
  # tuning
  tuned_mars <- train(
    x = subset(TND_train_ctr, select = c(-Y,-V)),
    y = TND_train_ctr$V,
    method = "earth",
    trControl = trainControl(method = "cv", number = 10),
    tuneGrid = hyper_grid
  ...
  TNDmod_g <- earth(
    V ~ .,  
    data = subset(TND_train_ctr, select = -Y),
    glm=list(family=binomial),
    nprune = as.numeric(tuned_mars$bestTune)[1],
    degree = as.numeric(tuned_mars$bestTune)[2]
  )

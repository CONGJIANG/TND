...
mod_eifsub <- function(TNDdf_train1, TNDdf_train2, deWet = c("PS", "Out")){
  ...
  Q1 <- predict(mu$mod,newdata=as.data.frame(cbind(V=1,C=TNDdf_train2$C)),type="response")*w1
  Q0 <- predict(mu$mod,newdata=as.data.frame(cbind(V=0,C=TNDdf_train2$C)),type="response")*w0
  #2) m model
  m <- mod_m(TNDdf_train1)
  m=predict(m$mod,type="response", newdata=as.data.frame(cbind(C=TNDdf_train2$C,V=rep(1, nrow(TNDdf_train2)),Y=TNDdf_train2$Y)))
  #3) PS model
  g1Cont <- mod_g_control(TNDdf_train1)
  g1_v1<-predict(g1Cont$mod,type="response",newdata=as.data.frame(cbind(C=TNDdf_train2$C,V=rep(1, nrow(TNDdf_train2)),Y=TNDdf_train2$Y)))
  g1_v0 <- 1 - g1_v1
...
mod_EIF <- function(TNDdf_train, deWet = c("PS", "Out")){
  ...
  res <- nrow(TNDdf_train1)*res1/nrow(TNDdf_train) + nrow(TNDdf_train2)*res2/nrow(TNDdf_train)
}
...
mod_eifsub2 <- function(TNDdf_train1, TNDdf_train2, deWet = c("PS", "Out")){
  mu <- mod_mu(TNDdf_train1)
  mu1=predict(mu$mod,newdata=as.data.frame(cbind(V=1,C=TNDdf_train2$C)),type="response")
  mu0=predict(mu$mod,newdata=as.data.frame(cbind(V=0,C=TNDdf_train2$C)),type="response")
  g1Cont <- mod_g_control(TNDdf_train1)
  g1_v1<-predict(g1Cont$mod,type="response",newdata=as.data.frame(cbind(C=TNDdf_train2$C,V=rep(1, nrow(TNDdf_train2)),Y=TNDdf_train2$Y)));
  g1_v0 <- 1 - g1_v1
...
res1<-read.table("/Users/congjiang/Downloads/TNDStudy1hal2000.txt",header=F)
res1 <- na.omit(res1)
head(res1)
dim(res1)

...
datagen<-function(seed=sample(1:1000000,size=1),ssize=500,popsize=1500000,OR_C=3,OR_WI=1,OR_WC=5,OR_H=1.5,em=3,cfV0=F,cfV1=F,return_full=F){
  set.seed(seed)
  #generate data
  C<-rnorm(n=popsize, sd = 2)
  ...
  if(cfV0==F&cfV1==F){
    V<-rbinom(prob=plogis(-5- 0.5*C + C^2 + sin(C)),size=1,n=popsize) #prevalence is around 0.61
  }
  mean(V)
  ...
  #Infected with COVID
  Infec_COVID<- rbinom(prob=plogis( -log(OR_C)*V -2 - C - cos(C) - 0.2*C^3 - 0.2*exp(C) + em*V*C+log(3)*U2*(1.5-V)-2*U1), size=1,n=popsize) #0.009
  #mean(Infec_COVID)
